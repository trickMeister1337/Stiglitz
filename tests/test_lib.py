...
class TestReportShowsCriticality(unittest.TestCase):
    """Task 12 — o card de finding exibe vetor CVSS, score Environmental,
    classe do ativo e proveniência; aviso quando score_source == 'estimated'."""

    def test_card_includes_cvss_vector_and_source(self):
        from report_generator import _render_finding_card
        f = {
            "sev": "High", "tool": "stiglitz", "title": "IDOR",
            "target": "https://x.com/api", "type": "stiglitz",
            "detail": "", "count": 1, "endpoints": [],
            "cvss_vector": "CVSS:3.1/AV:N/AC:L/PR:L/UI:N/S:U/C:H/I:N/A:N/E:H/RC:C/CR:H/IR:H",
            "cvss_environmental": 8.1,
            "score_source": "catalog",
            "asset_class": "cde",
        }
        h = _render_finding_card(1, f)
        self.assertIn("CVSS:3.1", h)
        self.assertIn("8.1", h)
        self.assertIn("cde", h)
        self.assertIn("catalog", h)
        # Sem flag de estimado quando source_source != 'estimated'
        self.assertNotIn("estimado", h)

    def test_card_estimated_warning(self):
        from report_generator import _render_finding_card
        f = {
            "sev": "Medium", "tool": "nikto", "title": "Config Issue",
            "target": "https://x.com/", "type": "stiglitz",
            "detail": "", "count": 1, "endpoints": [],
            "cvss_vector": "CVSS:3.1/AV:N/AC:H/PR:N/UI:R/S:U/C:L/I:L/A:N",
            "cvss_environmental": 3.4,
            "score_source": "estimated",
            "asset_class": "default",
        }
        h = _render_finding_card(1, f)
        self.assertIn("estimado", h)

    def test_card_without_cvss_omits_row(self):
        from report_generator import _render_finding_card
        f = {
            "sev": "Low", "tool": "nikto", "title": "Info",
            "target": "https://x.com/", "type": "stiglitz",
            "detail": "", "count": 1, "endpoints": [],
        }
        h = _render_finding_card(1, f)
        self.assertNotIn("CVSS:3.1", h)


class TestRiskScoreUsesEnvironmentalBand(unittest.TestCase):
    """Task 11 — compute_risk recebe contagens por banda FINAL (Environmental)."""
    ...
    def test_compute_risk_uses_environmental_band_counts(self):
        ...
        out = compute_risk(stats, {}, [], [])
        self.assertGreaterEqual(out["risk"], 70)  # 1 crítico → banda CRÍTICA

    def test_enrich_findings_severity_reflects_environmental_band_not_tool(self):
        """Contagens por f['severity'] devem refletir a banda Environmental (Task 11).

        Um finding com vuln_class='missing_security_header' e severity_tool='high'
        recebe banda Environmental 'low' (vetor AC:H/UI:R/C:L/I:L → base baixo).
        A contagem de stats['high'] deve ser 0 e stats['low'] deve ser 1,
        provando que o loop de stats lê f['severity'] (Environmental), não severity_tool.
        """
        import criticality as cr
        findings = [
            {
                "name": "Missing CSP",
                "severity": "high",          # severidade da ferramenta (alta)
                "vuln_class": "missing_security_header",
                "url": "https://www.example.com/",
            }
        ]
        cr.enrich_findings(findings)
        f = findings[0]

        # Contrato Task 11: severity reflete banda Environmental
        self.assertEqual(f["severity_tool"], "high",
                         "severity_tool deve guardar a severidade original da ferramenta")
        self.assertEqual(f["severity"], "low",
                         "severity deve ser a banda Environmental calculada (não a da ferramenta)")
        self.assertEqual(f["score_source"], "catalog")

        # Simular o loop de stats do stiglitz_report.py usando f['severity']:
        stats = {"critical": 0, "high": 0, "medium": 0, "low": 0, "info": 0}
        for finding in findings:
            sev = finding["severity"]   # banda Environmental
            if sev in stats:
                stats[sev] += 1

        self.assertEqual(stats["low"], 1,
                         "O finding de missing_security_header deve contar em 'low' (Environmental)")
        self.assertEqual(stats["high"], 0,
                         "Não deve contar em 'high' (severity_tool não deve ser usada)")


class TestSarifEnvironmental(unittest.TestCase):
    """Task 13 — security-severity usa cvss_environmental quando presente."""

    def test_security_severity_uses_environmental(self):
        import sys as _sys, os as _os
        _root = _os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))
        _sys.path.insert(0, _os.path.join(_root, "lib", "report"))
        from report.sarif import build_sarif
        findings = [{"name": "IDOR", "severity": "high", "cvss_environmental": 8.1,
                     "url": "https://x.com/api"}]
    ...
    def test_security_severity_falls_back_to_cvss_field(self):
        from report.sarif import build_sarif
        findings = [{"name": "SQLi", "severity": "critical", "cvss": 9.8,
                     "url": "https://x.com/api"}]
        s = build_sarif(findings, "https://x.com")
        rule = s["runs"][0]["tool"]["driver"]["rules"][0]
        self.assertEqual(rule["properties"]["security-severity"], "9.8")

    def test_security_severity_falls_back_to_sev_map(self):
        from report.sarif import build_sarif
        findings = [{"name": "Info", "severity": "medium", "url": "https://x.com"}]
        s = build_sarif(findings, "https://x.com")
        rule = s["runs"][0]["tool"]["driver"]["rules"][0]
        self.assertEqual(rule["properties"]["security-severity"], "5.0")

    def test_security_severity_env_overrides_cvss_field(self):
        from report.sarif import build_sarif
        # cvss_environmental (7.5) deve prevalecer sobre cvss sintético (9.8)
        findings = [{"name": "X", "severity": "critical", "cvss": 9.8,
                     "cvss_environmental": 7.5, "url": "https://x.com"}]
        s = build_sarif(findings, "https://x.com")
        rule = s["runs"][0]["tool"]["driver"]["rules"][0]
