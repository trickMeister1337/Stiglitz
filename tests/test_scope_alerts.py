...
def test_filter_alerts_drops_out_of_scope():
    ...
    alerts = [
        {"name": "CSP", "url": "https://transactions.example.com/api/v1/Transactions"},
        {"name": "HSTS", "url": "https://www.google.com/async/folae?async=_fmt:pb"},
    ]
    out = scope.filter_alerts_in_scope(alerts, "transactions.example.com")
    urls = [a["url"] for a in out]
    assert "https://transactions.example.com/api/v1/Transactions" in urls
...
def test_filter_alerts_keeps_subdomain_and_urlless():
    alerts = [
        {"name": "Z", "url": "https://sub.transactions.example.com/x"},
        {"name": "W"},  # sem url → mantém (consumidor aplica o alvo como fallback)
        {"name": "U", "uri": "https://transactions.example.com/y"},  # campo 'uri'
    ]
    out = scope.filter_alerts_in_scope(alerts, "transactions.example.com")
    assert len(out) == 3
