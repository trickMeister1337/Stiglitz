...
| `js_chunks.py` | Helpers puros consumidos por `js_analysis.py`: `webpack_chunk_urls` resolve os **lazy chunks** do manifest webpack (`i.u`: id→hash/nome + public path) — código real de SPAs que não aparece como `<script src>` (antes só a entry era varrida); `is_js_payload` é a **guarda anti-FP de catch-all** (SPA devolve index.html 200/text/html p/ path inexistente → nunca varrer HTML como JS, origem de falso "secret em bundle"); `discover_js_urls` é o **crawl de descoberta determinístico** (fronteira em lista, não `set` — `set.pop()` varia por `PYTHONHASHSEED` e, sob o teto de páginas, fazia cada scan varrer páginas/chunks diferentes → findings de SCA divergentes; `fetch_fn` injetável, sem rede própria). Lógica pura + CLI (`chunks <entry.js> <url>`) |
| `host_lock.sh` | **Lock por-host** (sourceado pelo `stiglitz.sh`): serializa dois `stiglitz.sh <mesmo-host>` concorrentes, que disputariam o daemon ZAP (porta fixa) e o store de estado → relatórios contaminados/divergentes (o lock por-OUTDIR não cobre isso — é único por timestamp). PID-file + `flock` de vida curta na seção crítica check-and-write (fechado antes de retornar → não vaza fd p/ os filhos do scan); `kill -9` deixa lock stale recuperável (detectado via `kill -0`), não bloqueio eterno. Escape hatches: `STIGLITZ_NO_HOSTLOCK=1`, `STIGLITZ_HOSTLOCK_WAIT=<s>`, `STIGLITZ_LOCK_DIR` |
...
| `apm_probe.py` | Detecção de Elastic APM Server com ingestão/config sem auth (Fase 3, ao lado do `monitoring_check`). Fecha gap: o ZAP spider só acha a raiz e o nuclei não tem template (APM é API pura, exige POST NDJSON). Probe **não-destrutivo**: POST de NDJSON **inválido** em `/intake/v2/events` (→ 400 de schema, nunca indexa) + GET `/config/v1/agents`; classifica por **400/200-sem-auth vs 401-com-token** (`classify_intake`/`classify_config`/`looks_like_apm`, puros). Fail-safe: só emite com fingerprint de APM + CONFIRMED → `raw/apm_probe.json` (classes `apm_unauth_ingestion` High / `apm_central_config_exposed` Medium, CWE-306). Lógica pura + `send_fn` injetável + CLI |
...
| `finding_quality.py` | Heurísticas puras de qualidade de findings (consumidas pelo `stiglitz_report.py`): `is_low_value_zap_alert` (rebaixa FP previsível — Timestamp/Suspicious-Comments disclosure em `.js`/`.css` de terceiro — p/ info, sem deixar a reclassificação CWE inflá-lo); `enrich_vulnerable_js`/`identify_js_library` (dá nome+versão ao genérico "Vulnerable JS Library" do ZAP, ex.: Swagger UI 3.1.6); `coverage_gap_finding` (agora alimenta o sinal da seção Coverage & Blind Spots via `coverage_report`, não mais emitido como finding); `swagger_exposure_severity` (spec OpenAPI público → medium em escopo CDE, info fora); `severity_counts` (contagem canônica por severidade, espelha o findings.json — sumário do terminal e export usam a mesma base). Lógica pura |
| `zap_authgate.py` | Detector de FP "JWT-gate" para alertas ZAP active-scan: ZAP deslogado em API auth-gated recebe 401/403 antes da query → heurístico lê dois 401 como "manipulation" → falso CRITICAL (SQLi/XSS). `extract_status`/`needs_status`/`enrich` (resolve `messageId`→`attack_status` via `core/view/message`, fetch injetável) + `is_authgated_fp` (`attack` não-vazio E status ∈ {401,403}). Fase 9 enriquece `zap_alerts.json` com `attack_status`; Fase 11 (`stiglitz_report.py`) particiona inline por `is_authgated_fp` → seção "Deferred — Requires Authenticated Retest" + bloco `deferred_findings`, fora das contagens/criticidade. **Fail-safe** (status ausente → não defere); **auto-ajuste** (scan logado → ataque 200 → não defere); aditivo (vira lead de retest, nunca some). Lógica pura + runner injetável + CLI (`enrich <zap_alerts.json> <zap_base_url>`) |
...
| `compliance_map.py` | Crosswalk CWE → frameworks (OWASP Top 10 2021, ASVS 4.0, PCI DSS 4.0.1, ISO 27001, NIST 800-53) — puro lookup, consumido pelo `stiglitz_report.py`. `tag_finding` anexa `compliance` ao finding; `pci_posture(findings, coverage)` e `owasp_posture(findings, coverage)` dão veredito por requisito/categoria (**FOUND/TESTED/NOT-COVERED** ou PASS/FAIL/N-A) com base em quais capacidades de scan rodaram (`coverage`). **NÃO compõe criticidade** — é só rótulo/cobertura de apresentação, separado do score CVSS+KEV+EPSS. OWASP coverage é sempre visível; PCI é gated ao CDE. Lógica pura |
