...
- **Temporal** — reflete o status de confirmação que o toolkit já produz:
  - Confirmado (PoC validado / OOB): `E:H` (Exploit Code Maturity High), `RC:C` (Report Confidence Confirmed).
  - Potencial (não confirmado): `E:P` (Proof-of-Concept), `RC:R` (Reasonable).
  - `RL` (Remediation Level): `U` (Unavailable) por padrão; ajustável quando conhecido.
...
### 3.2 Inferência de classe (padrões Bee2pay) — override sempre vence

| Padrão no host/path (regex, case-insensitive) | Classe inferida |
|---|---|
| `card`, `pan`, `tokenize`, `vcn`, `3ds`, `acs`, `maxipago`, `braspag` | `cde` |
...
| `fraudguard`, `buyer`, `reservation`, `hotel`, `chargeback`, `receivable`, `notification`, `email-service`, `simples-nacional` | `pii` |
...
- **Classifier**: host/path → classe (padrões Bee2pay + override; override vence).
