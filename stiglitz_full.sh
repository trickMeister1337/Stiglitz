...
def status_badge(ran, issues=None):
    if not ran:
        return '<span style="background:#e0e0e0;color:#888;padding:2px 8px;border-radius:4px;font-size:11px">SKIPPED</span>'
...
rows = [
    ("🔍 OSINT", osint_dir, status_badge(bool(osint_dir), osint_sub),
     f"{osint_sub} subdomains · {osint_leaks} leaks", link(osint_report, "OSINT Report")),
    ("📡 Stiglitz", scan_dir, status_badge(bool(scan_dir), scan_find),
     f"{scan_find} findings · {scan_conf} confirmed · {scan_cves} CVEs", link(scan_report, "Stiglitz Report")),
    ("⚔ Stiglitz RED", red_dir, status_badge(bool(red_dir), red_total),
     f"SQLi:{red_sqli} · XSS:{red_xss} · Brute:{red_brute}", link(red_report, "RED Report")),
]
...
<div class="header">
  <h1>🎯 Stiglitz FULL — Consolidated Report</h1>
  <p>{html.escape(target)} &nbsp;·&nbsp; Profile: {html.escape(profile.upper())} &nbsp;·&nbsp; {ts_str} &nbsp;·&nbsp; Total duration: {total_time} &nbsp;·&nbsp; CONFIDENTIAL</p>
</div>
<div class="container">
  <div class="kpis">
    <div class="kpi"><div class="n">{osint_sub}</div><div class="l">Subdomains</div></div>
    <div class="kpi"><div class="n">{scan_find}</div><div class="l">Scan Findings</div></div>
    <div class="kpi"><div class="n" style="color:{'#b34e4e' if red_total>0 else '#2e7d32'}">{red_total}</div><div class="l">Confirmed Exploits</div></div>
    <div class="kpi"><div class="n">{scan_cves}</div><div class="l">Detected CVEs</div></div>
    <div class="kpi"><div class="n">{osint_leaks}</div><div class="l">Leaked Credentials</div></div>
  </div>
  <div class="card">
    <div class="card-title">Execution Pipeline</div>
    <table>
      <thead><tr>
        <th>Phase</th><th>Status</th><th>Summary</th><th>Report</th><th>Directory</th>
      </tr></thead>
...
<div class="footer">
  Generated by Stiglitz FULL v{html.escape(version)} · {ts_str} · Restricted to authorized teams · CONFIDENTIAL
</div>
</body></html>"""
